...
/***/ "./resources/js/admin/libs/helpers.js":
/*!********************************************!*\
  !*** ./resources/js/admin/libs/helpers.js ***!
  \********************************************/
...
var Helpers = {
  ...
  parseUrl: function parseUrl() {
    ...
    var row = null === url ? window.location.href : url;
    ...
    if (typeof temp !== 'undefined') {
      var _temp$split = temp.split('#');
      var _temp$split2 = _slicedToArray(_temp$split, 2);
      params = _temp$split2[0];
      hash = _temp$split2[1];
      params = params.split('&').reduce(function (sum, curr) {
        var temp = curr.split('=');
   ...
   * @returns {string}
   */
  setRequestOrderParams: function setRequestOrderParams(url, order) {
    // Parse current URL
    var urlInfo = Helpers.parseUrl(url);
    ...
    return "".concat(urlInfo.path, "?").concat(new URLSearchParams(urlInfo.search).toString());
  },
  /**
  ...
  toAscii: function toAscii(str) {
    ...
    str = str.trim().split('');
    var result = '';
    for (var _char in str) {
      result += typeof converter[str[_char]] !== 'undefined' ? converter[str[_char]] : str[_char];
    }
...
/* harmony import */ var _libs_helpers__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./libs/helpers */ "./resources/js/admin/libs/helpers.js");


window.$.axios = axios__WEBPACK_IMPORTED_MODULE_0__["default"];
window.Helpers = _libs_helpers__WEBPACK_IMPORTED_MODULE_1__.Helpers;
window.$.axios.interceptors.request.use(function (config) {
  ...
  var $jwt = $('meta[name="jwt-token"]');
  if ($jwt.length) {
    config.headers.Authorization = "Bearer ".concat($jwt.attr('content'));
  }
  return config;
});
})();
